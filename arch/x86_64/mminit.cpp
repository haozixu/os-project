...
__fixed_mapping_info fixed_mapping_info;
...
static void add_bios_data_area_mapping()
{
	...
	bios_area.set(bios_data_area::START, __va(bios_data_area::START), 4KiB | 1, 32);
...
static void add_apic_mmio_mmaping()
{
	...
	mmio_area.set(io_apic::DEFAULT_BASE, __va(io_apic::DEFAULT_BASE), 2MiB | 1, 2); 
	// range 0xfec00000 ~ 0xff000000 2 2MiB pages cache disable
	fixed_mapping_info.replace(mmio_area);
}

inline void __fixed_mapping_info::print() const
{
...
inline int __fixed_mapping_info::insert(unsigned index, const mapping_descriptor& md)
{
	if (index + 1 >= MAX_NR_DESCS)
		return -1;
...
inline int __fixed_mapping_info::remove(unsigned index)
{
	if (index >= MAX_NR_DESCS)
		return -1;
...
inline void __fixed_mapping_info::replace(const mapping_descriptor& md)
{
	unsigned i;
...
void __init init_kernel_physical_mapping()
{
	...
	use_2m_page:
		...
		addr = __pa(reinterpret_cast<unsigned long>(pdt));
	...
	skip_pdt_alloc:
		...
		addr = __pa(reinterpret_cast<unsigned long>(pt));
		for (j = 0; j < nr_pgt; ++j, addr += 4KiB) {
	...
	skip_pt_alloc:
		...
		for (j = 0; j < md.nr_pages; ++j, addr += PAGE_SIZE) {
			if (is_kernel_text(__va(addr), __va(addr + PAGE_SIZE))) {
				flags &= ~PG_RW;
			} else if (is_kernel_rodata(__va(addr), __va(addr + PAGE_SIZE))) {
				flags &= ~PG_RW;
				flags |= PG_XD;
			} else if (is_kernel_data(__va(addr), __va(addr + PAGE_SIZE))) {
				flags |= PG_XD;
			}
