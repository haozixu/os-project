/*
 *	arch/x86_64/memory/e820map.cpp
 *
...
namespace ARCH {
	...
	{		
		...
		if (err)
			return err;
	#else
		init(mmap_info.original_map, mmap_info.nr_entries);
	#endif
	...
	unsigned long e820::end_address(uint32_t type, unsigned long limit)
	{
